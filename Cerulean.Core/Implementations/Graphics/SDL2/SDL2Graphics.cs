...
namespace Cerulean.Core
{
    internal class SDL2Graphics : IGraphics
    {
        ...
        public void SetRenderArea(Size renderArea, int x, int y, int? offsetX, int? offsetY)
        {
            SDL_Rect rect = new()
            {
                w = renderArea.W,
            ...
            _globalY = y;
            if (offsetX.HasValue)
                _offsetX = offsetX.Value;
            if (offsetY.HasValue)
                _offsetY = offsetY.Value;
        ...
        public void DrawRectangle(int x, int y, Size size)
        {
            SDL_Rect rect = new()
            {
                x = x + _offsetX + _globalX,
                y = y + _offsetY + _globalY,
                w = size.W,
                h = size.H
            };
        ...
        public void DrawText(int x, int y, string text, string fontName, string fontStyle, int fontPointSize, Color color, uint textWrap, double angle)
        {
            ...
            if (!_textureCache.TryGetTexture(textFingerprint, out var textTexture))
            {
                ...
                destRect.x = x + _offsetX + _globalX;
                destRect.y = y + _offsetY + _globalY;
