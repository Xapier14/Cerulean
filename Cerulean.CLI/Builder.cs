...
namespace Cerulean.CLI
{
    ...
    public class Builder
    {
        ...
        public void ProcessXElement(StringBuilder stringBuilder, int depth, XElement element, string parent = "")
        {
            ...
            if (!result)
            {
                ColoredConsole.WriteLine($"[$cyan^{handler.GetType()}$r^] Handler returned an error while parsing an XElement.");
            }
        }
        ...
        private void ProcessLayout(XElement layoutElement, BuilderContext context)
        {
            ...
            {
                ProcessXElement(stringBuilder, 3, xElement);
            }
        ...
        private void RegisterHandler(string? handlerName, ConstructorInfo? handlerConstructor)
        {
            ...
            {
                ColoredConsole.WriteLine($"[$cyan^Builder.RegisterHandler()$r^] Could not load element handler \"{handlerConstructor?.Name}\".");
            }
        }
    }
}
